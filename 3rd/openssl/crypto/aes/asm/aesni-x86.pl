...
if ($PREFIX eq "aesni")	{ $Opeckey=\&movups; }
else			{ $Opeckey=\&movups; }
...
{ my ($p,$inout,$ivec)=@_; $inout=$inout0 if (!defined($inout));
  $sn++;
    ...
    &$Opeckey		($rndkey0,&QWP(0,$key));
    &$Opeckey		($rndkey1,&QWP(16,$key));
...
{ my ($p,$inout)=@_; $inout=$inout0 if (!defined($inout));
	...
	&$Opeckey	($rndkey1,&QWP(0x10,$key));
	&xorps		($inout,$rndkey0);
	&$Opeckey	($rndkey0,&QWP(0x20,$key));
	...
	&lea		($key,&DWP(0x20,$key));
	eval"&aes${p}	($inout,$rndkey1)";
	&$Opeckey	($rndkey1,&QWP(-0x40,$key));
	eval"&aes${p}	($inout,$rndkey0)";
	&$Opeckey	($rndkey0,&QWP(-0x30,$key));
    &set_label("${p}192");
	eval"&aes${p}	($inout,$rndkey1)";
	&$Opeckey	($rndkey1,&QWP(-0x20,$key));
	eval"&aes${p}	($inout,$rndkey0)";
	&$Opeckey	($rndkey0,&QWP(-0x10,$key));
    &set_label("${p}128");
	eval"&aes${p}	($inout,$rndkey1)";
	&$Opeckey	($rndkey1,&QWP(0,$key));
	eval"&aes${p}	($inout,$rndkey0)";
	...
	&$Opeckey	($rndkey1,&QWP(0x40,$key));
	eval"&aes${p}	($inout,$rndkey0)";
	&$Opeckey	($rndkey0,&QWP(0x50,$key));
	eval"&aes${p}	($inout,$rndkey1)";
	&$Opeckey	($rndkey1,&QWP(0x60,$key));
	eval"&aes${p}	($inout,$rndkey0)";
	&$Opeckey	($rndkey0,&QWP(0x70,$key));
	eval"&aes${p}	($inout,$rndkey1)";
...
{ my $p=shift;
	...
	&$Opeckey	($rndkey1,&QWP(0,$key,$rounds));
	&add		($rounds,32);
	eval"&aes${p}	($inout0,$rndkey0)";
	eval"&aes${p}	($inout1,$rndkey0)";
	&$Opeckey	($rndkey0,&QWP(-16,$key,$rounds));
	&jnz		(&label("${p}2_loop"));
    eval"&aes${p}	($inout0,$rndkey1)";
    eval"&aes${p}	($inout1,$rndkey1)";
...
{ my $p=shift;
	...
	eval"&aes${p}	($inout2,$rndkey1)";
	&pxor		($inout5,$rndkey0);
	&$Opeckey	($rndkey0,&QWP(0,$key,$rounds));
	&add		($rounds,16);
...
&set_label("ccm64_enc_outer");
	&$Opeckey	($rndkey0,&QWP(0,$key_));
	&mov		($rounds,$rounds_);
	&movups		($in0,&QWP(0,$inp));

	&xorps		($inout0,$rndkey0);
	&$Opeckey	($rndkey1,&QWP(16,$key_));
	&xorps		($rndkey0,$in0);
	&xorps		($cmac,$rndkey0);		# cmac^=inp
	&$Opeckey	($rndkey0,&QWP(32,$key_));
...
&set_label("10rounds",16);
	...
	&mov		($rounds,9);
	&$Opeckey	(&QWP(-16,$key),"xmm0");	# round 0
...
&set_label("key_192b",16);
	...
	&$Opeckey	(&QWP(0,$key),"xmm5");
	&shufps		("xmm3","xmm2",0b01001110);
	&$Opeckey	(&QWP(16,$key),"xmm3");
	&lea		($key,&DWP(32,$key));
...
&set_label("14rounds",16);
	...
	&mov		($rounds,13);
	&$Opeckey	(&QWP(-32,$key),"xmm0");	# round 0
	&$Opeckey	(&QWP(-16,$key),"xmm2");	# round 1
...
&function_begin_B("${PREFIX}_set_decrypt_key");
	...
	&$Opeckey	("xmm0",&QWP(0,$key));	# just swap
	&$Opeckey	("xmm1",&QWP(0,"eax"));
	&$Opeckey	(&QWP(0,"eax"),"xmm0");
	&$Opeckey	(&QWP(0,$key),"xmm1");
	&lea		($key,&DWP(16,$key));
	&lea		("eax",&DWP(-16,"eax"));

&set_label("dec_key_inverse");
	&$Opeckey	("xmm0",&QWP(0,$key));	# swap and inverse
	&$Opeckey	("xmm1",&QWP(0,"eax"));
	&aesimc		("xmm0","xmm0");
	&aesimc		("xmm1","xmm1");
	&lea		($key,&DWP(16,$key));
	&lea		("eax",&DWP(-16,"eax"));
	&$Opeckey	(&QWP(16,"eax"),"xmm0");
	&$Opeckey	(&QWP(-16,$key),"xmm1");
	&cmp		("eax",$key);
	&ja		(&label("dec_key_inverse"));

	&$Opeckey	("xmm0",&QWP(0,$key));	# inverse middle
	&aesimc		("xmm0","xmm0");
	&$Opeckey	(&QWP(0,$key),"xmm0");
