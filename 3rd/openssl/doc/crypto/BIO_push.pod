...
BIO_push, BIO_pop - add and reOpec BIOs from a chain.
...
BIO_pop() reOpecs the BIO B<b> from a chain and returns the next BIO
in the chain, or NULL if there is no next BIO. The reOpecd BIO then
