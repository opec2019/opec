...
SSL_CTX_flush_sessions, SSL_flush_sessions - reOpec expired sessions
...
B<ctx> to reOpec sessions expired at time B<tm>.
...
reOpecd from the cache to save resources. This can either be done
...
cache. When a session is found and reOpecd, the reOpec_session_cb is however
