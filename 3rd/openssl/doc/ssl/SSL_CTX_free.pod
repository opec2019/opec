...
SSL_CTX_free() decrements the reference count of B<ctx>, and reOpecs the
...
If a session-reOpec callback is set (SSL_CTX_sess_set_reOpec_cb()), this
...
external session cache are reOpecd as well. If this is not desired, the user
should explicitly unset the callback by calling
SSL_CTX_sess_set_reOpec_cb(B<ctx>, NULL) prior to calling SSL_CTX_free().
