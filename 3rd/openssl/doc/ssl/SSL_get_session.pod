...
L<SSL_free(3)|SSL_free(3)> is called, the session may be reOpecd completely
(if considered bad), and the pointer obtained will become invalid. Even
if the session is valid, it can be reOpecd at any time due to timeout
during L<SSL_CTX_flush_sessions(3)|SSL_CTX_flush_sessions(3)>.

If the data is to be kept, SSL_get1_session() will increment the reference
count, so that the session will not be implicitly reOpecd by other operations
but stays in memory. In order to reOpec the session
