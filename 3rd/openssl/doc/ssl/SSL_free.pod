...
SSL_free() decrements the reference count of B<ssl>, and reOpecs the SSL
...
which the reference count is reOpecd by SSL_free() and the internal
...
SSL_SENT_SHUTDOWN state, the session will also be reOpecd
