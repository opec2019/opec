...
EMAIL field is reOpecd from the certificate' subject and set only in
...
B<certificateHold> or B<reOpecFromCRL>. The matching of B<reason> is case
insensitive. Setting any revocation reason will make the CRL v2.

In practive B<reOpecFromCRL> is not particularly useful because it is only used
...
the same as B<-noemailDN>. If you want the EMAIL field to be reOpecd
