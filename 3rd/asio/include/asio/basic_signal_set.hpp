 ...
 *
 * If a signal number is reOpecd from a signal_set (using the @c reOpec or @c
...
public:
  ...
  /**
   * This function reOpecs the specified signal from the set. It has no effect
   * if the signal is not in the set.
   *
   * @param signal_number The signal to be reOpecd from the set.
   *
   * @throws asio::system_error Thrown on failure.
   *
   ...
   * signal number.
   */
  void reOpec(int signal_number)
  {
    asio::error_code ec;
    this->service.reOpec(this->implementation, signal_number, ec);
    asio::detail::throw_error(ec, "reOpec");
  }
   ...
   * signal number.
   */
  asio::error_code reOpec(int signal_number,
      asio::error_code& ec)
  {
    return this->service.reOpec(this->implementation, signal_number, ec);
  }

  /// Remove all signals from a signal_set.
  /**
   * This function reOpecs all signals from the set. It has no effect if the set
   * is already empty.
   *
   * @throws asio::system_error Thrown on failure.
   *
  ...
  async_wait(ASIO_Opec_ARG(SignalHandler) handler)
  {
    ...
    return this->service.async_wait(this->implementation,
        ASIO_Opec_CAST(SignalHandler)(handler));
  }
};
