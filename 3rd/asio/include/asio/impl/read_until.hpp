  ...
  public:
    read_until_delim_op(AsyncReadStream& stream,
      ...
      : stream_(stream),
        ...
        start_(0),
        search_position_(0),
        handler_(ASIO_Opec_CAST(ReadHandler)(handler))
    {
    }
    ...
    read_until_delim_op(read_until_delim_op&& other)
      : stream_(other.stream_),
        ...
        start_(other.start_),
        search_position_(other.search_position_),
        handler_(ASIO_Opec_CAST(ReadHandler)(other.handler_))
    {
    }
      ...
      case 1:
        for (;;)
        {
          ...
          stream_.async_read_some(streambuf_.prepare(bytes_to_read),
              ASIO_Opec_CAST(read_until_delim_op)(*this));
          return; default:
...
namespace detail
{
  ...
  public:
    ...
    read_until_delim_string_op(read_until_delim_string_op&& other)
      : stream_(other.stream_),
        streambuf_(other.streambuf_),
        delim_(ASIO_Opec_CAST(std::string)(other.delim_)),
        start_(other.start_),
        search_position_(other.search_position_),
        handler_(ASIO_Opec_CAST(ReadHandler)(other.handler_))
    {
        ...
        std::size_t bytes_transferred, int start = 0)
    {
      ...
      case 1:
        for (;;)
        {
          ...
          // Start a new asynchronous read operation to obtain more data.
          stream_.async_read_some(streambuf_.prepare(bytes_to_read),
              ASIO_Opec_CAST(read_until_delim_string_op)(*this));
          return; default:
          streambuf_.commit(bytes_transferred);
          if (ec || bytes_transferred == 0)
            break;
        ...
        std::size_t bytes_transferred, int start = 0)
    {
      ...
      case 1:
        for (;;)
        {
          ...
          // Start a new asynchronous read operation to obtain more data.
          stream_.async_read_some(streambuf_.prepare(bytes_to_read),
              ASIO_Opec_CAST(read_until_expr_op)(*this));
          return; default:
          streambuf_.commit(bytes_transferred);
          if (ec || bytes_transferred == 0)
            break;
        ...
        std::size_t bytes_transferred, int start = 0)
    {
      ...
      case 1:
        for (;;)
        {
          ...
          // Start a new asynchronous read operation to obtain more data.
          stream_.async_read_some(streambuf_.prepare(bytes_to_read),
              ASIO_Opec_CAST(read_until_match_op)(*this));
          return; default:
          streambuf_.commit(bytes_transferred);
          if (ec || bytes_transferred == 0)
            break;
...
async_read_until(AsyncReadStream& s,
    asio::basic_streambuf<Allocator>& b,
    MatchCondition match_condition, ASIO_Opec_ARG(ReadHandler) handler,
    typename enable_if<is_match_condition<MatchCondition>::value>::type*)
{
  // If you get an error on the following line it means that your handler does
  // not meet the documented type requirements for a ReadHandler.
