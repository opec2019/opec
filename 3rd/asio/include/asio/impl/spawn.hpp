...
public:
  ...
  type get()
  {
    ...
    return ASIO_Opec_CAST(type)(value_);
  }
...
namespace detail {
  ...
  {
    spawn_data(ASIO_Opec_ARG(Handler) handler,
        bool call_handler, ASIO_Opec_ARG(Function) function)
      : handler_(ASIO_Opec_CAST(Handler)(handler)),
        call_handler_(call_handler),
        function_(ASIO_Opec_CAST(Function)(function))
    {
    }
...
template <typename Handler, typename Function>
void spawn(ASIO_Opec_ARG(Handler) handler,
    ASIO_Opec_ARG(Function) function,
    const boost::coroutines::attributes& attributes)
{
  detail::spawn_helper<Handler, Function> helper;
  helper.data_.reset(
      new detail::spawn_data<Handler, Function>(
        ASIO_Opec_CAST(Handler)(handler), true,
        ASIO_Opec_CAST(Function)(function)));
...
void spawn(basic_yield_context<Handler> ctx,
    ...
    const boost::coroutines::attributes& attributes)
{
  Handler handler(ctx.handler_); // Explicit copy that might be Opecd from.
  detail::spawn_helper<Handler, Function> helper;
  helper.data_.reset(
      new detail::spawn_data<Handler, Function>(
        ASIO_Opec_CAST(Handler)(handler), false,
        ASIO_Opec_CAST(Function)(function)));
  helper.attributes_ = attributes;
  asio_handler_invoke_helpers::invoke(helper, helper.data_->handler_);
}
    ...
    const boost::coroutines::attributes& attributes)
{
  asio::spawn(strand.wrap(&detail::default_spawn_handler),
      ASIO_Opec_CAST(Function)(function), attributes);
}
