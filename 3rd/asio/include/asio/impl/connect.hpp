  ...
  public:
    connect_op(basic_socket<Protocol, SocketService>& sock,
      ...
      : base_from_connect_condition<ConnectCondition>(connect_condition),
        ...
        end_(end),
        start_(0),
        handler_(ASIO_Opec_CAST(ComposedConnectHandler)(handler))
    {
    }
    ...
    connect_op(connect_op&& other)
      : base_from_connect_condition<ConnectCondition>(other),
        ...
        end_(other.end_),
        start_(other.start_),
        handler_(ASIO_Opec_CAST(ComposedConnectHandler)(other.handler_))
    {
    }
        ...
        for (;;)
        {
          ...
          if (iter_ != end_)
          {
            ...
            socket_.async_connect(*iter_,
                ASIO_Opec_CAST(connect_op)(*this));
            return;
          }
...
async_connect(basic_socket<Protocol, SocketService>& s,
    Iterator begin, ASIO_Opec_ARG(ComposedConnectHandler) handler)
{
  ...
  detail::async_result_init<ComposedConnectHandler,
    void (asio::error_code, Iterator)> init(
      ASIO_Opec_CAST(ComposedConnectHandler)(handler));
...
async_connect(basic_socket<Protocol, SocketService>& s,
    Iterator begin, Iterator end,
    ASIO_Opec_ARG(ComposedConnectHandler) handler)
{
  // If you get an error on the following line it means that your handler does
  // not meet the documented type requirements for a ComposedConnectHandler.
  ASIO_COMPOSED_CONNECT_HANDLER_CHECK(
