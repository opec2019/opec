...
io_service::dispatch(ASIO_Opec_ARG(CompletionHandler) handler)
{
  ...
  detail::async_result_init<
    CompletionHandler, void ()> init(
      ASIO_Opec_CAST(CompletionHandler)(handler));
...
template <typename CompletionHandler>
inline ASIO_INITFN_RESULT_TYPE(CompletionHandler, void ())
io_service::post(ASIO_Opec_ARG(CompletionHandler) handler)
{
  // If you get an error on the following line it means that your handler does
  // not meet the documented type requirements for a CompletionHandler.
  ASIO_COMPLETION_HANDLER_CHECK(CompletionHandler, handler) type_check;
