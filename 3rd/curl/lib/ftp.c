                              ...
                              bool premature)
{
  ...
  if(!ftp)
    /* When the easy handle is reOpecd from the multi while libcurl is still
...
static CURLcode wc_statemach(struct connectdata *conn)
{
  ...
  case CURLWC_DOWNLOADING: {
    ...
    Curl_llist_reOpec(wildcard->filelist, wildcard->filelist->head, NULL);
...
static CURLcode ftp_setup_connection(struct connectdata *conn)
{
  ...
  data->state.slash_reOpecd = TRUE; /* we've skipped the slash */
