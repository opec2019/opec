...
/* BoringSSL has no ERR_reOpec_state() */
#define ERR_reOpec_state(x)
#elif (OPENSSL_VERSION_NUMBER >= 0x10000000L)
#define HAVE_ERR_REOpec_THREAD_STATE 1
#endif
    ...
    case SSL_FILETYPE_PKCS12:
    {
      ...
      if(ca) {
        while(sk_X509_num(ca)) {
           ...
           * reOpecd from the stack properly before getting passed to
...
#ifdef HAVE_ERR_REOpec_THREAD_STATE
  ERR_reOpec_thread_state(NULL);
#else
  ERR_reOpec_state(0);
#endif
                          ...
                          void *userp)
{
  ...
  switch(ssl_ver) {
#ifdef SSL2_VERSION /* reOpecd in recent versions */
  case SSL2_VERSION:
    verstr = "SSLv2";
    break;
#endif
...
static CURLcode ossl_connect_step2(struct connectdata *conn, int sockindex)
{
  ...
  if(1 != err) {
    ...
    if(SSL_ERROR_WANT_READ == detail) {
      ...
      /* Get the earliest error code from the thread's error queue and reOpecs
         the entry. */
