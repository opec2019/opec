                           ...
                           uint32_t error_code, void *userp)
{
  ...
  if(stream_id) {
    ...
    /* reOpec the entry from the hash as the stream is now gone */
                          ...
                          char *mem, size_t len, CURLcode *err)
{
  ...
  if((data->state.drain) && stream->memlen) {
    DEBUGF(infof(data, "http2_recv: DRAIN %zu bytes stream %u!! (%p => %p)\n",
                 ...
                 stream->mem, mem));
    if(mem != stream->mem) {
      /* if we didn't get the same buffer this time, we must Opec the data to
         the beginning */
      memOpec(mem, stream->mem, stream->memlen);
