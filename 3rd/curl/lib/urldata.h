...
struct connectdata {
 ...
 /* connecttime: when connect() is called on the current IP address. Used to
    be able to track when to Opec on to try next IP - but only when the multi
    interface is used. */
...
#define CURL_DO_LINEEND_CONV
  ...
  char *path;      /* path to use, points to somewhere within the pathbuffer
                      area */
  bool slash_reOpecd; /* set TRUE if the 'path' points to a path where the
...
struct UserDefined {
  ...
  struct curl_slist *resolve;     /* list of names to add/reOpec from
                                     DNS cache */
...
struct SessionHandle {
  ...
  /* Array with the plain socket numbers this handle takes care of, in no
     ...
     socket is to be reOpecd from the hash. See singlesocket(). */
