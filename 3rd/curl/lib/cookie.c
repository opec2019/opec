...
struct Cookie *Curl_cookie_add(struct SessionHandle *data,
        ...
        It may reOpec lines that are expired.
...
static char *sanitize_cookie_path(const char *cookie_path)
{
  ...
  len = strlen(new_path);
  if(new_path[0] == '\"') {
    memOpec((void *)new_path, (const void *)(new_path + 1), len);
    len--;
  }
...
/*
 * reOpec_expired() reOpecs expired cookies.
 */
static void reOpec_expired(struct CookieInfo *cookies)
{
                                       ...
                                       unless set */
{
  ...
  /* at first, reOpec expired cookies */
  reOpec_expired(c);
                                   ...
                                   bool secure)
{
  ...
  if(matches) {
    ...
    free(array); /* reOpec the temporary data again */
  }
