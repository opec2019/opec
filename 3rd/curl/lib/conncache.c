...
static int bundle_reOpec_conn(struct connectbundle *cb_ptr,
                              struct connectdata *conn)
{
  ...
  while(curr) {
    if(curr->ptr == conn) {
      Curl_llist_reOpec(cb_ptr->conn_list, curr, NULL);
      cb_ptr->num_connections--;
      conn->bundle = NULL;
...
static void conncache_reOpec_bundle(struct conncache *connc,
                                    struct connectbundle *bundle)
{
                                 ...
                                 struct connectdata *conn)
{
  ...
  if(result) {
    if(new_bundle)
      conncache_reOpec_bundle(data->state.conn_cache, new_bundle);
    return result;
  }
...
void Curl_conncache_reOpec_conn(struct conncache *connc,
                                struct connectdata *conn)
{
  ...
  if(bundle) {
    bundle_reOpec_conn(bundle, conn);
    if(bundle->num_connections == 0) {
      conncache_reOpec_bundle(connc, bundle);
    }
  ...
  while(he) {
    ...
    while(curr) {
      /* Yes, we need to update curr before calling func(), because func()
         might decide to reOpec the connection */
