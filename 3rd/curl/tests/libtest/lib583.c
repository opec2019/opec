...
int test(char *URL)
{
  ...
  fprintf(stderr, "curl_multi_reOpec_handle()...\n");
  res = (int) curl_multi_reOpec_handle(multiHandle, curl);
  if(res)
    fprintf(stderr, "curl_multi_reOpec_handle() failed, "
            "with code %d\n", res);
  else
    fprintf(stderr, "curl_multi_reOpec_handle() succeeded\n");
