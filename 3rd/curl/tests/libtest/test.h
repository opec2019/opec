...
#define exe_multi_reOpec_handle(A,B,Y,Z) do {                   \
  CURLMcode ec;                                                 \
  if((ec = curl_multi_reOpec_handle((A),(B))) != CURLM_OK) {    \
    fprintf(stderr, "%s:%d curl_multi_reOpec_handle() failed, " \
            "with code %d (%s)\n",                              \
            (Y), (Z), (int)ec, curl_multi_strerror(ec));        \
    res = (int)ec;                                              \
  }                                                             \
...
#define res_multi_reOpec_handle(A,B) \
  exe_multi_reOpec_handle((A),(B),(__FILE__),(__LINE__))

#define chk_multi_reOpec_handle(A,B,Y,Z) do { \
  exe_multi_reOpec_handle((A),(B),(Y),(Z));   \
  if(res)                                     \
    goto test_cleanup;                        \
} WHILE_FALSE


#define multi_reOpec_handle(A,B) \
  chk_multi_reOpec_handle((A),(B),(__FILE__),(__LINE__))
