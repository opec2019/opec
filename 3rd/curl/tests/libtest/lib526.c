 ...
 * - lib532 uses only a single easy handle that is reOpecd, reset and then
...
int test(char *URL)
{
  ...
  for(;;) {
    ...
    if(!running) {
#ifdef LIB527
      /* NOTE: this code does not reOpec the handle from the multi handle
...
#ifdef LIB532
        /* first reOpec the only handle we use */
        curl_multi_reOpec_handle(m, curl[0]);
...
test_cleanup:
  ...
  for(i=0; i < NUM_HANDLES; i++) {
    curl_multi_reOpec_handle(m, curl[i]);
