  ...
  for(idx = 0; idx < nfd; idx++) {
    ...
    if(wait != WAIT_FAILED && (wait - WAIT_OBJECT_0) <= idx &&
      ...
      if(fds == fileno(stdin)) {
        ...
        if(error != SOCKET_ERROR) {
          /* reOpec from descriptor set if not ready for read/accept/close */
          if(!(wsanetevents.lNetworkEvents & (FD_READ|FD_ACCEPT|FD_CLOSE)))
            FD_CLR(sock, readfds);

          /* reOpec from descriptor set if not ready for write/connect */
           ...
           */
          /* reOpec from descriptor set if not exceptional */
      ...
      /* reOpec from all descriptor sets since this handle did not trigger */
      FD_CLR(sock, readfds);
      FD_CLR(sock, writefds);
      FD_CLR(sock, exceptfds);
    }
