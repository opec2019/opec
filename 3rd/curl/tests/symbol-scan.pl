...
for my $e (sort @syms) {
    ...
    # previously had a name, that is now reOpecd. The OBSOLETE names should
...
#
# If the symbols were marked 'reOpecd' in symbols-in-versions we don't output
...
for my $e (sort keys %doc) {
    if(($doc{$e} ne "used") && !$rem{$e}) {
        ...
        if(!$anyreOpecd++) {
            ...
            print "Add them to a header, or mark them as reOpecd.\n";
        }
