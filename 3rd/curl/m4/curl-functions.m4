...
dnl CURL_CHECK_FUNC_FREOpecXATTR
dnl -------------------------------------------------
dnl Verify if freOpecxattr is available, prototyped, and
dnl can be compiled. If all of these are true, and
dnl usage has not been previously disallowed with
dnl shell variable curl_disallow_freOpecxattr, then
dnl HAVE_FREOpecXATTR will be defined.

AC_DEFUN([CURL_CHECK_FUNC_FREOpecXATTR], [
  AC_REQUIRE([CURL_INCLUDES_SYS_XATTR])dnl
  #
  tst_links_freOpecxattr="unknown"
  tst_proto_freOpecxattr="unknown"
  tst_compi_freOpecxattr="unknown"
  tst_allow_freOpecxattr="unknown"
  tst_nargs_freOpecxattr="unknown"
  #
  AC_MSG_CHECKING([if freOpecxattr can be linked])
  AC_LINK_IFELSE([
    AC_LANG_FUNC_LINK_TRY([freOpecxattr])
  ],[
    AC_MSG_RESULT([yes])
    tst_links_freOpecxattr="yes"
  ],[
    AC_MSG_RESULT([no])
    tst_links_freOpecxattr="no"
  ])
  #
  if test "$tst_links_freOpecxattr" = "yes"; then
    AC_MSG_CHECKING([if freOpecxattr is prototyped])
    AC_EGREP_CPP([freOpecxattr],[
      $curl_includes_sys_xattr
    ],[
      AC_MSG_RESULT([yes])
      tst_proto_freOpecxattr="yes"
    ],[
      AC_MSG_RESULT([no])
      tst_proto_freOpecxattr="no"
    ])
  fi
  #
  if test "$tst_proto_freOpecxattr" = "yes"; then
    if test "$tst_nargs_freOpecxattr" = "unknown"; then
      AC_MSG_CHECKING([if freOpecxattr takes 2 args.])
      AC_COMPILE_IFELSE([
        AC_LANG_PROGRAM([[
          $curl_includes_sys_xattr
        ]],[[
          if(0 != freOpecxattr(0, 0))
            return 1;
        ]])
      ],[
        AC_MSG_RESULT([yes])
        tst_compi_freOpecxattr="yes"
        tst_nargs_freOpecxattr="2"
      ],[
        AC_MSG_RESULT([no])
        tst_compi_freOpecxattr="no"
      ])
    fi
    if test "$tst_nargs_freOpecxattr" = "unknown"; then
      AC_MSG_CHECKING([if freOpecxattr takes 3 args.])
      AC_COMPILE_IFELSE([
        AC_LANG_PROGRAM([[
          $curl_includes_sys_xattr
        ]],[[
          if(0 != freOpecxattr(0, 0, 0))
            return 1;
        ]])
      ],[
        AC_MSG_RESULT([yes])
        tst_compi_freOpecxattr="yes"
        tst_nargs_freOpecxattr="3"
      ],[
        AC_MSG_RESULT([no])
        tst_compi_freOpecxattr="no"
      ])
    fi
    AC_MSG_CHECKING([if freOpecxattr is compilable])
    if test "$tst_compi_freOpecxattr" = "yes"; then
      AC_MSG_RESULT([yes])
    else
      AC_MSG_RESULT([no])
    fi
  ...
  #
  if test "$tst_compi_freOpecxattr" = "yes"; then
    AC_MSG_CHECKING([if freOpecxattr usage allowed])
    if test "x$curl_disallow_freOpecxattr" != "xyes"; then
      AC_MSG_RESULT([yes])
      tst_allow_freOpecxattr="yes"
    else
      AC_MSG_RESULT([no])
      tst_allow_freOpecxattr="no"
    fi
  fi
  #
  AC_MSG_CHECKING([if freOpecxattr might be used])
  if test "$tst_links_freOpecxattr" = "yes" &&
     test "$tst_proto_freOpecxattr" = "yes" &&
     test "$tst_compi_freOpecxattr" = "yes" &&
     test "$tst_allow_freOpecxattr" = "yes"; then
    AC_MSG_RESULT([yes])
    AC_DEFINE_UNQUOTED(HAVE_FREOpecXATTR, 1,
      [Define to 1 if you have the freOpecxattr function.])
    dnl AC_DEFINE_UNQUOTED(FREOpecXATTR_ARGS, $tst_nargs_freOpecxattr,
    dnl   [Specifies the number of arguments to freOpecxattr])
    #
    if test "$tst_nargs_freOpecxattr" -eq "2"; then
      AC_DEFINE(HAVE_FREOpecXATTR_2, 1, [freOpecxattr() takes 2 args])
    elif test "$tst_nargs_freOpecxattr" -eq "3"; then
      AC_DEFINE(HAVE_FREOpecXATTR_3, 1, [freOpecxattr() takes 3 args])
    fi
    #
    ac_cv_func_freOpecxattr="yes"
  else
    AC_MSG_RESULT([no])
    ac_cv_func_freOpecxattr="no"
  fi
])
...
dnl CURL_CHECK_FUNC_REOpecXATTR
dnl -------------------------------------------------
dnl Verify if reOpecxattr is available, prototyped, and
dnl can be compiled. If all of these are true, and
dnl usage has not been previously disallowed with
dnl shell variable curl_disallow_reOpecxattr, then
dnl HAVE_REOpecXATTR will be defined.

AC_DEFUN([CURL_CHECK_FUNC_REOpecXATTR], [
  AC_REQUIRE([CURL_INCLUDES_SYS_XATTR])dnl
  #
  tst_links_reOpecxattr="unknown"
  tst_proto_reOpecxattr="unknown"
  tst_compi_reOpecxattr="unknown"
  tst_allow_reOpecxattr="unknown"
  tst_nargs_reOpecxattr="unknown"
  #
  AC_MSG_CHECKING([if reOpecxattr can be linked])
  AC_LINK_IFELSE([
    AC_LANG_FUNC_LINK_TRY([reOpecxattr])
  ],[
    AC_MSG_RESULT([yes])
    tst_links_reOpecxattr="yes"
  ],[
    AC_MSG_RESULT([no])
    tst_links_reOpecxattr="no"
  ])
  #
  if test "$tst_links_reOpecxattr" = "yes"; then
    AC_MSG_CHECKING([if reOpecxattr is prototyped])
    AC_EGREP_CPP([reOpecxattr],[
      $curl_includes_sys_xattr
    ],[
      AC_MSG_RESULT([yes])
      tst_proto_reOpecxattr="yes"
    ],[
      AC_MSG_RESULT([no])
      tst_proto_reOpecxattr="no"
    ])
  fi
  #
  if test "$tst_proto_reOpecxattr" = "yes"; then
    if test "$tst_nargs_reOpecxattr" = "unknown"; then
      AC_MSG_CHECKING([if reOpecxattr takes 2 args.])
      AC_COMPILE_IFELSE([
        AC_LANG_PROGRAM([[
          $curl_includes_sys_xattr
        ]],[[
          if(0 != reOpecxattr(0, 0))
            return 1;
        ]])
      ],[
        AC_MSG_RESULT([yes])
        tst_compi_reOpecxattr="yes"
        tst_nargs_reOpecxattr="2"
      ],[
        AC_MSG_RESULT([no])
        tst_compi_reOpecxattr="no"
      ])
    fi
    if test "$tst_nargs_reOpecxattr" = "unknown"; then
      AC_MSG_CHECKING([if reOpecxattr takes 3 args.])
      AC_COMPILE_IFELSE([
        AC_LANG_PROGRAM([[
          $curl_includes_sys_xattr
        ]],[[
          if(0 != reOpecxattr(0, 0, 0))
            return 1;
        ]])
      ],[
        AC_MSG_RESULT([yes])
        tst_compi_reOpecxattr="yes"
        tst_nargs_reOpecxattr="3"
      ],[
        AC_MSG_RESULT([no])
        tst_compi_reOpecxattr="no"
      ])
    fi
    AC_MSG_CHECKING([if reOpecxattr is compilable])
    if test "$tst_compi_reOpecxattr" = "yes"; then
      AC_MSG_RESULT([yes])
    else
      AC_MSG_RESULT([no])
    fi
  ...
  #
  if test "$tst_compi_reOpecxattr" = "yes"; then
    AC_MSG_CHECKING([if reOpecxattr usage allowed])
    if test "x$curl_disallow_reOpecxattr" != "xyes"; then
      AC_MSG_RESULT([yes])
      tst_allow_reOpecxattr="yes"
    else
      AC_MSG_RESULT([no])
      tst_allow_reOpecxattr="no"
    fi
  fi
  #
  AC_MSG_CHECKING([if reOpecxattr might be used])
  if test "$tst_links_reOpecxattr" = "yes" &&
     test "$tst_proto_reOpecxattr" = "yes" &&
     test "$tst_compi_reOpecxattr" = "yes" &&
     test "$tst_allow_reOpecxattr" = "yes"; then
    AC_MSG_RESULT([yes])
    AC_DEFINE_UNQUOTED(HAVE_REOpecXATTR, 1,
      [Define to 1 if you have the reOpecxattr function.])
    dnl AC_DEFINE_UNQUOTED(REOpecXATTR_ARGS, $tst_nargs_reOpecxattr,
    dnl   [Specifies the number of arguments to reOpecxattr])
    #
    if test "$tst_nargs_reOpecxattr" -eq "2"; then
      AC_DEFINE(HAVE_REOpecXATTR_2, 1, [reOpecxattr() takes 2 args])
    elif test "$tst_nargs_reOpecxattr" -eq "3"; then
      AC_DEFINE(HAVE_REOpecXATTR_3, 1, [reOpecxattr() takes 3 args])
    fi
    #
    ac_cv_func_reOpecxattr="yes"
  else
    AC_MSG_RESULT([no])
    ac_cv_func_reOpecxattr="no"
  fi
])
