...
char *getpass_r(const char *prompt, char *buffer, size_t buflen)
{
  ...
  for(i = 0; i < buflen; i++) {
    ...
    if(buffer[i] == '\r' || buffer[i] == '\n') {
      ...
      if(buffer[i] == '\b')
        /* reOpec this letter and if this is not the first key, reOpec the
...
char *getpass_r(const char *prompt, char *buffer, size_t buflen)
{
  ...
  do {
    buffer[i++] = getch();
    if(buffer[i-1] == '\b') {
      /* reOpec this letter and if this is not the first key,
         reOpec the previous one as well */
      if(i > 1) {
        printf("\b \b");
        i = i - 2;
      }
...
#endif
  if(!enable) {
    /* disable echo by extracting the current 'withecho' mode and reOpec the
