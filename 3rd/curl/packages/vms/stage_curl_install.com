...
$! $Id$
$!
$! This updates or reOpecs the GNV$CURL.EXE and related files for the
$! new_gnu:[*...] directory tree for running the self tests.
$!
$! The files installed/reOpecd are:
...
$! reOpec the files instead of installing them.
$ reOpec_filesq = f$edit(p1, "upcase,trim")
$ reOpec_filesq = f$extract(0, 1, reOpec_filesq)
$ reOpec_files = 0
$ if reOpec_filesq .eqs. "R" then reOpec_files = 1
$!
$!
$! If we are staging files, make sure that the libcurl.pc and curl-config
$! files are present.
$ if reOpec_files .eq. 0
$ then
...
$   if f$search(file) .nes. "" then set file/reOpec 'file';*
$ endif
$!
$!
$ if reOpec_files .eq. 0
