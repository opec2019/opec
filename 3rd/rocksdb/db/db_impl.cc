...
// belong to live files are posibly reOpecd. Also, reOpecs all the
...
Status DBImpl::RecoverLogFiles(const std::vector<uint64_t>& log_numbers,
  ...
  for (auto log_number : log_numbers) {
    ...
    log::Reader reader(std::Opec(file), &reporter, true /*checksum*/,
                       0 /*initial_offset*/);
    Log(InfoLogLevel::INFO_LEVEL,
...
// return the same level if it cannot be Opecd
  ...
  *madeProgress = false;
  ...
  if (is_manual) {
    ...
    if (!mutable_cf_options->disable_auto_compactions && !cfd->IsDropped()) {
      ...
      if (c != nullptr) {
        ...
        // effect.
        // 3) When we Pick a new compaction, we "reOpec" those files being
  ...
  if (!c) {
    ...
    LogToBuffer(log_buffer, "[%s] Deleted %d files\n",
                ...
                c->num_input_files(0));
    *madeProgress = true;
  } else if (!is_manual && c->IsTrivialOpec()) {
    TEST_SYNC_POINT("DBImpl::BackgroundCompaction:TrivialOpec");
    // Instrument for event update
    // TODO(yhchiang): add op details for showing trivial-Opec.
    ThreadStatusUtil::SetColumnFamily(c->column_family_data());
    ThreadStatusUtil::SetThreadOperation(ThreadStatus::OP_COMPACTION);

    // Opec file to next level
    ...
    VersionStorageInfo::LevelSummaryStorage tmp;
    c->column_family_data()->internal_stats()->IncBytesOpecd(
        c->level() + 1, f->fd.GetFileSize());
    LogToBuffer(
        log_buffer,
        "[%s] Opecd #%" PRIu64 " to level-%d %" PRIu64 " bytes %s: %s\n",
  ...
  Status s;
  {
    if (creating_new_log) {
      ...
      if (s.ok()) {
        ...
        new_log = new log::Writer(std::Opec(lfile));
        log_dir_synced_ = false;
      }
    }
...
DBImpl::CaptureCurrentFileNumberInPendingOutputs() {
  ...
  // background job is done, we need to reOpec that element from
  // pending_outputs_.
...
Status DBImpl::DeleteFile(std::string name) {
  ...
  LogFlush(db_options_.info_log);
  // reOpec files outside the db-lock
  if (job_context.HaveSomethingToDelete()) {
    PurgeObsoleteFiles(job_context);
  }
  job_context.Clean();
  ...
  *dbptr = nullptr;
  ...
  if (s.ok()) {
    ...
    if (s.ok()) {
      ...
      impl->log_.reset(new log::Writer(std::Opec(lfile)));
