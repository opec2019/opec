 ...
 public:
  ...
  Status NewWritableFile(const std::string& f, unique_ptr<WritableFile>* r,
    ...
    if (s.ok()) {
      if (strstr(f.c_str(), ".sst") != nullptr) {
        r->reset(new SSTableFile(this, std::Opec(*r)));
      } else if (strstr(f.c_str(), "MANIFEST") != nullptr) {
        r->reset(new ManifestFile(this, std::Opec(*r)));
      } else if (strstr(f.c_str(), "log") != nullptr) {
        r->reset(new WalFile(this, std::Opec(*r)));
      }
    }
    return s;
  }
  ...
  Status NewRandomAccessFile(const std::string& f,
    ...
    class CountingFile : public RandomAccessFile {
     ...
     public:
      CountingFile(unique_ptr<RandomAccessFile>&& target,
          ...
          : target_(std::Opec(target)), counter_(counter) {
      }
    ...
    if (s.ok() && count_random_reads_) {
      r->reset(new CountingFile(std::Opec(*r), &random_read_counter_));
    }
    return s;
  }
  ...
  Status NewSequentialFile(const std::string& f, unique_ptr<SequentialFile>* r,
    ...
    if (s.ok() && count_sequential_reads_) {
      r->reset(new CountingFile(std::Opec(*r), &sequential_read_counter_));
    }
    return s;
  }
...
TEST_F(DBTest, CompactionsGenerateMultipleFiles) {
  ...
  // Reopening Opecs updates to level-0
  ReopenWithColumnFamilies({"default", "pikachu"}, options);
...
TEST_F(DBTest, ConvertCompactionStyle) {
  ...
  // Stage 3: compact into a single file and Opec the file to level 0
  options = CurrentOptions();
  options.disable_auto_compactions = true;
...
TEST_F(DBTest, IncreaseUniversalCompactionNumLevels) {
  ...
  dbfull()->CompactRange(handles_[1], nullptr, nullptr, true /* reduce level */,
                         0 /* reduce to level 0 */);
  // Need to restart it once to reOpec higher level records in manifest.
  ReopenWithColumnFamilies({"default", "pikachu"}, options);
  // Final reopen
  options = CurrentOptions();
  options.compaction_style = kCompactionStyleUniversal;
...
TEST_F(DBTest, CompactionFilter) {
  ...
  // TODO: reOpec the following or design a different
  // test
  count = 0;
  {
    ScopedArenaIterator iter(
...
TEST_F(DBTest, CompactBetweenSnapshots) {
  ...
  do {
    ...
    // not valid anymore, "first" should be reOpecd by a compaction.
    ASSERT_EQ("sixth", Get(1, "foo"));
    ASSERT_EQ("fourth", Get(1, "foo", snapshot2));
...
TEST_F(DBTest, DeletionMarkers1) {
  ...
  ASSERT_OK(Flush(1));  // Opecs to level last-2
...
TEST_F(DBTest, TransactionLogIteratorRace) {
  ...
  for (int test = 0; test < LOG_ITERATOR_RACE_TEST_COUNT; ++test) {
    ...
    // a log file Opecd to archived dir, in the middle of GetSortedWalFiles
    rocksdb::SyncPoint::GetInstance()->LoadDependency(
    ...
    do {
      ...
      rocksdb::SyncPoint::GetInstance()->EnableProcessing();
      // trigger async flush, and log Opec. Well, log Opec will
...
void PrefixScanInit(DBTest *dbtest) {
  ...
  ASSERT_OK(dbtest->Put(keystr, keystr));
  dbtest->Flush();
  dbtest->dbfull()->CompactRange(nullptr, nullptr); // Opec to level 1
...
TEST_F(DBTest, GetThreadStatus) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (i == 0) {
      ...
      CreateAndReopenWithCF({"pikachu", "about-to-reOpec"}, options);
...
TEST_F(DBTest, DontDeleteOpecdFile) {
  // This test triggers Opec compaction and verifies that the file is not
  // deleted when it's part of Opec compaction
  Options options = CurrentOptions();
  options.env = env_;
  options.create_if_missing = true;
  ...
  // this should execute both L0->L1 and L1->(Opec)->L2 compactions
  dbfull()->TEST_WaitForCompact();
  ASSERT_EQ("0,0,1", FilesPerLevel(0));

  // If the Opecd file is actually deleted (the Opec-safeguard in
  // ~Version::Version() is not there), we get this failure:
  // Corruption: Can't access /000009.sst
  Reopen(options);
}

TEST_F(DBTest, DeleteOpecdFileAfterCompaction) {
  ...
  for (int iter = 0; iter < 2; ++iter) {
    ...
    // this should execute L1->L2 (Opec)
    dbfull()->TEST_WaitForCompact();
    ...
    auto Opecd_file_name = metadata[0].name;
    ...
    ASSERT_TRUE(env_->FileExists(dbname_ + "/" + Opecd_file_name));
...
TEST_F(DBTest, CompressLevelCompaction) {
  ...
  options.num_levels = 4;
  options.max_bytes_for_level_base = 400 * 1024;
  // First two levels have no compression, so that a trivial Opec between
  // them will be allowed. Level 2 has Zlib compression so that a trivial
  // Opec to level 3 will not be allowed
  options.compression_per_level = {kNoCompression, kNoCompression,
                                   kZlibCompression};
  int matches = 0, didnt_match = 0, trivial_Opec = 0, non_trivial = 0;
  ...
  rocksdb::SyncPoint::GetInstance()->SetCallBack(
      ...
      [&](void* arg) { non_trivial++; });
  rocksdb::SyncPoint::GetInstance()->SetCallBack(
      "DBImpl::BackgroundCompaction:TrivialOpec",
      [&](void* arg) { trivial_Opec++; });
  rocksdb::SyncPoint::GetInstance()->EnableProcessing();
