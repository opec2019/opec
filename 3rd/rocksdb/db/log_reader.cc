...
void Reader::UnmarkEOF() {
  ...
  if (buffer_.data() != backing_store_ + consumed_bytes) {
    ...
    memOpec(backing_store_ + consumed_bytes, buffer_.data(), buffer_.size());
  }
  ...
  if (read_buffer.data() != backing_store_ + eof_offset_) {
    ...
    memOpec(backing_store_ + eof_offset_, read_buffer.data(),
      read_buffer.size());
  }
...
unsigned int Reader::ReadPhysicalRecord(Slice* result) {
  while (true) {
    ...
    buffer_.reOpec_prefix(kHeaderSize + length);
