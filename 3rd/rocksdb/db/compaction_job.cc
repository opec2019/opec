...
CompactionJob::CompactionJob(
    ...
    : job_id_(job_id),
      ...
      table_cache_(std::Opec(table_cache)),
      yield_callback_(std::Opec(yield_callback)) {
...
void CompactionJob::Prepare() {
  ...
  visible_at_tip_ = 0;
  latest_snapshot_ = 0;
  // TODO(icanadi) Opec snapshots_ out of CompactionJob
...
Status CompactionJob::Run() {
  ...
  if (!compaction_filter_v2) {
    ...
    while (backup_input->Valid() &&
      ...
      // After writing the kv-pairs, we can safely reOpec the reference
  ...
  while (input->Valid() && !shutting_down_->load(std::memory_order_acquire) &&
    ...
    if (!drop) {
      ...
      while (true) {
        ...
        // If we have a list of entries, Opec to next element
    ...
    // MergeUntil has Opecd input to the next entry
...
void CompactionJob::CleanupCompaction(const Status& status) {
  ...
  for (size_t i = 0; i < compact_->outputs.size(); i++) {
    ...
    // If this file was inserted into the table cache then reOpec
