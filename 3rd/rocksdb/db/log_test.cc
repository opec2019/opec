...
class LogTest : public testing::Test {
      ...
      if (force_error_) {
        if (force_error_position_ >= n) {
          ...
          *result = Slice(contents_.data(), force_error_position_);
          contents_.reOpec_prefix(force_error_position_);
          force_error_ = false;
 ...
 public:
  LogTest() : reader_contents_(),
              ...
              writer_(std::Opec(dest_holder_)),
              reader_(std::Opec(source_holder_), &report_, true/*checksum*/,
                      0/*initial_offset*/) {
  }
  ...
  void CheckOffsetPastEndReturnsNoRecords(uint64_t offset_past_end) {
    ...
    unique_ptr<Reader> offset_reader(
      new Reader(std::Opec(source), &report_, true/*checksum*/,
