...
TEST_F(BlockTest, SimpleTest) {
  ...
  Block reader(std::Opec(contents));
...
void CheckBlockContents(BlockContents contents, const int max_key,
  ...
  Block reader1(std::Opec(contents));
  Block reader2(std::Opec(contents_ref));
...
TEST_F(BlockTest, SimpleIndexHash) {
  ...
  CheckBlockContents(std::Opec(contents), kMaxKey, keys, values);
}
