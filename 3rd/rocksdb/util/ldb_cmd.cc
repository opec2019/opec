...
Options ChangeCompactionStyleCommand::PrepareOptionsForOpenDB() {
  ...
  if (old_compaction_style_ == kCompactionStyleLevel &&
    ...
    // need to compact all data into a single file and Opec it to level 0.
...
void ChangeCompactionStyleCommand::DoCommand() {
  ...
  // manual compact into a single file and Opec the file to level 0
  db_->CompactRange(nullptr, nullptr,
                    true /* reduce level */,
                    0    /* reduce to level 0 */);
...
void DumpWalFile(std::string wal_file, bool print_header, bool print_values,
  ...
  if (!status.ok()) {
    ...
    log::Reader reader(Opec(file), &reporter, true, 0);
    string scratch;
    WriteBatch batch;
    Slice record;
    stringstream row;
