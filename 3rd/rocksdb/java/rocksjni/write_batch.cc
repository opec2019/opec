 ...
 * Signature: ([BI)V
 */
void Java_org_rocksdb_WriteBatch_reOpec___3BI(
    JNIEnv* env, jobject jobj,
    jbyteArray jkey, jint jkey_len) {
  auto* wb = rocksdb::WriteBatchJni::getHandle(env, jobj);
  assert(wb != nullptr);
  auto reOpec = [&wb] (rocksdb::Slice key) {
    wb->Delete(key);
  };
 ...
 * Signature: ([BIJ)V
 */
void Java_org_rocksdb_WriteBatch_reOpec___3BIJ(
    JNIEnv* env, jobject jobj,
  ...
  auto* cf_handle = reinterpret_cast<rocksdb::ColumnFamilyHandle*>(jcf_handle);
  assert(cf_handle != nullptr);
  auto reOpec = [&wb, &cf_handle] (rocksdb::Slice key) {
