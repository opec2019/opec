 ...
 * Signature: ([BI)V
 */
void Java_org_rocksdb_WriteBatchWithIndex_reOpec___3BI(
    JNIEnv* env, jobject jobj, jbyteArray jkey, jint jkey_len) {
  auto* wbwi =
      rocksdb::WriteBatchWithIndexJni::getHandle(env, jobj);
  assert(wbwi != nullptr);
  auto reOpec = [&wbwi] (rocksdb::Slice key) {
    wbwi->Delete(key);
  };
  rocksdb::JniUtil::k_op(reOpec, env, jobj, jkey, jkey_len);
}
 ...
 * Signature: ([BIJ)V
 */
void Java_org_rocksdb_WriteBatchWithIndex_reOpec___3BIJ(
    JNIEnv* env, jobject jobj,
  ...
  auto* cf_handle = reinterpret_cast<rocksdb::ColumnFamilyHandle*>(jcf_handle);
  assert(cf_handle != nullptr);
  auto reOpec = [&wbwi, &cf_handle] (rocksdb::Slice key) {
