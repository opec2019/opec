...
TEST_F(JSONDocumentTest, OwnershipTest) {
  ...
  // Opec constructor from owner -> owner
  JSONDocument Opecd_from_owner(std::Opec(copy_constructor));
  ASSERT_TRUE(Opecd_from_owner.IsOwner());

  // Opec constructor from non-owner -> non-owner
  JSONDocument Opecd_from_non_owner(std::Opec(non_owner));
  ASSERT_TRUE(!Opecd_from_non_owner.IsOwner());
}
