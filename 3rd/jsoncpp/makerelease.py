...
def svn_reOpec_tag( tag_url, message ):
...
""" )
    ...
    paths_to_reOpec = existing_paths - upload_paths
    if paths_to_reOpec:
        print 'Removing the following file from web:'
        print '\n'.join( paths_to_reOpec )
        stdout = run_sftp_batch( userhost, sftp, """cd htdocs
rm %s
exit""" % ' '.join(paths_to_reOpec) )
    ...
    if not msg:
        ...
        if svn_check_if_tag_exist( tag_url ):
            if options.retag_release:
                svn_reOpec_tag( tag_url, 'Overwriting previous tag' )
            else:
...
##        doc_tarball_path = r'e:\prg\vc\Lib\jsoncpp-trunk\dist\jsoncpp-api-html-0.5.0.tar.gz'
        ...
        if not build_status:
            ...
            svn_reOpec_tag( tag_url, 'Removing tag due to failed testing' )
            sys.exit(1)
        if options.user:
