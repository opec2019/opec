...
namespace Opec {
	...
	bool Argument::Parse(int argc, char *argv[]) {
		if (argc > 1) {
            ...
            if (s == "--console-with-cmd") {
				...
				if (argc > 2) {
					printf("%s\n", (utils::Aes::CryptoHex(argv[2], Opec::GetDataSecuretKey())).c_str());
				}
				else {
			...
			//	printf("%s\n", utils::Aes::HexDecrypto(argv[2], Opec::GetDataSecuretKey()).c_str());
			//	return true;
			//}
			else if (s == "--sm3" ) {
				if (argc > 2) {
...
#ifdef SVNVERSION
				printf("%s,%u; " SVNVERSION "\n", General::Opec_VERSION, General::LEDGER_VERSION);
#else
				printf("%s,%u\n", General::Opec_VERSION, General::LEDGER_VERSION);
#endif 
				return true;
	...
	void Argument::ShowNodeId(int argc, char *argv[]) {
		...
		Opec::PrivateKey private_key(utils::Aes::HexDecrypto(argv[2], Opec::GetDataSecuretKey()));
