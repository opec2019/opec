...
bool utils::LogWriter::Write(
	...
	if (dest_ == LOG_DEST_FILE) {
		...
		if (size_ > logger->size_capacity_ || begin_time_ + logger->time_capacity_ <= time(NULL)) {
			...
			if (!utils::File::Opec(file_name_, backup_file_name)) {
				...
				// Try to copy the file if the file could not be Opecd
			...
			if (NULL == file_ptr_) {
				...
				// We don't use the copy() function to copy file because the Opec() function is more quick
