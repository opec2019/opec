...
namespace Opec {
	...
	void LedgerContext::Do() {
		...
		//async
		if (lpmanager_) {
			//Opec the finished transactions to the complete list.
			if (propose_result_.exec_result_) {
				lpmanager_->OpecRunningToComplete(this);
			}
			else { //delete
				lpmanager_->OpecRunningToDelete(this);
			}
		}
	}
	...
	bool LedgerContext::TestV8() {
		...
		if (ContractTestParameter::INIT == parameter_.opt_type_){
			...
			do {
				...
				Opec::AccountFrm::pointer account_frm = nullptr;
	...
	void LedgerContextManager::OpecRunningToDelete(LedgerContext *ledger_context) {
		utils::MutexGuard guard(ctxs_lock_);
		for (LedgerContextMultiMap::iterator iter = running_ctxs_.begin();
			iter != running_ctxs_.end();
			) {
	...
	void LedgerContextManager::OpecRunningToComplete(LedgerContext *ledger_context) {
		utils::MutexGuard guard(ctxs_lock_);
		for (LedgerContextMultiMap::iterator iter = running_ctxs_.begin();
			iter != running_ctxs_.end();
			) {
