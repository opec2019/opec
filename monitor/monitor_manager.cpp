...
namespace Opec {
	MonitorManager::MonitorManager() : Network(SslParameter()) {
		...
		request_methods_[monitor::MONITOR_MSGTYPE_Opec] = std::bind(&MonitorManager::OnOpecStatus, this, std::placeholders::_1, std::placeholders::_2);
	...
	Opec::Connection * MonitorManager::CreateConnectObject(Opec::server *server_h, Opec::client *client_, Opec::tls_server *tls_server_h, 
		Opec::tls_client *tls_client_h, Opec::connection_hdl con, const std::string &uri, int64_t id) {
	...
	bool MonitorManager::OnMonitorHello(protocol::WsMessage &message, int64_t conn_id) {
		...
		do {
			...
			reg.set_blockchain_version(Opec::General::Opec_VERSION);
			reg.set_data_version(Opec::General::MONITOR_VERSION);
	...
	bool MonitorManager::OnOpecStatus(protocol::WsMessage &message, int64_t conn_id) {
		monitor::OpecStatus Opec_status;
		// Get the status of Opec
		GetOpecStatus(Opec_status);
		...
		// Send the response of Opec status
		if (NULL == monitor || !monitor->SendResponse(message, Opec_status.SerializeAsString(), ignore_ec)) {
			bret = false;
			LOG_ERROR("Failed to send Opec status from ip(%s) (%d:%s)", monitor->GetPeerAddress().ToIpPort().c_str(),
				ignore_ec.value(), ignore_ec.message().c_str());
		}
		return bret;
	}
	...
	bool MonitorManager::GetOpecStatus(monitor::OpecStatus &Opec_status) {
		...
		monitor::GlueManager *glue_manager = Opec_status.mutable_glue_manager();
		...
		monitor::PeerManager *peer_manager = Opec_status.mutable_peer_manager();
